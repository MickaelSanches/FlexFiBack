...
class BNPLController {
    ...
    async createBNPLSale(req, res) {
        ...
        try {
            const sale = await bnplService.createBNPLSale(sellerPubKey, buyerPubKey, amount, months);
            res.status(201).json({
            ...
            res.status(500).json({ error: "Erreur lors de la création de la vente BNPL" });
        }
    }
    ...
    async payBNPLInstallment(req, res) {
        ...
        try {
            const paymentResult = await bnplService.payBNPLInstallment(buyerPrivateKey, saleId);
            res.status(200).json({
            ...
            res.status(500).json({ error: "Erreur lors du paiement de la mensualité BNPL" });
        }
    }
    ...
    async getBNPLSaleDetails(req, res) {
        ...
        try {
            ...
            res.status(500).json({ error: "Erreur lors de la récupération des détails de la vente BNPL" });
        }
    }
    ...
    async getUserBNPLSales(req, res) {
        ...
        try {
            ...
            res.status(500).json({ error: "Erreur lors de la récupération des ventes BNPL de l'utilisateur" });
        }
    }
    ...
    async downloadSchedulePDF(req, res) {
        ...
        try {
            ...
            const schedule = await db.query('SELECT * FROM bnpl_schedules WHERE sale_id = $1 ORDER BY month_number', [saleId]);
            ...
            if (!Array.isArray(schedule.rows)) {
                ...
                return res.status(500).json({ error: "Erreur de format de données pour l'échéancier." });
            }
    
            // Générer le PDF en fonction du type (shopper ou merchant)
            const pdfBuffer = await pdfService.generateSchedulePDF(sale, schedule.rows, type);
    
            res.setHeader("Content-Type", "application/pdf");
            res.setHeader("Content-Disposition", `attachment; filename="schedule_${type}.pdf"`);
            res.send(pdfBuffer);
        } catch (error) {
