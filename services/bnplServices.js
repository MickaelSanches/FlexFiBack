...
class BNPLService {
  ...
  calculateBNPLDetails(amount, months) {
    ...
    console.log(
      `Frais Marchand: ${merchantFee}, Mensualité Shopper: ${monthlyPaymentShopper}`
    );
  ...
  generatePaymentSchedule(amount, months, saleId, startDate) {
    const schedule = [];
    const { shopperTotal, monthlyPaymentShopper } = this.calculateBNPLDetails(
      amount,
      months
    );
    ...
    console.log(
      `Montant Total: ${shopperTotal}, Mensualité: ${monthlyPaymentShopper}`
    );
    ...
    for (let i = 0; i < months; i++) {
      ...
      console.log(
        `Mensualité ${
          i + 1
        }: ${paymentAmount} (Échéance: ${paymentDate.toISOString()})`
      );
    ...
    for (const payment of paymentsDue.rows) {
      try {
        ...
        try {
          ...
          throw new Error(
            "Erreur lors du parsing de la clé privée de l’acheteur"
          );
        }
        ...
        if (!Array.isArray(buyerPrivateKey)) {
          throw new Error(
            "La clé privée de l’acheteur n’est pas un tableau valide."
          );
        }
        ...
        console.log(
          `Paiement BNPL réussi pour l'échéance ID ${payment.id} avec hash ${paymentHash}`
        );
      } catch (error) {
        console.error(
  ...
  async createBNPLSale(sellerPubKey, buyerPubKey, amount, months) {
    try {
      ...
      const { shopperFee, merchantFee, shopperTotal, monthlyPaymentShopper } =
        this.calculateBNPLDetails(amount, months);
      ...
      for (const payment of paymentSchedule) {
        ...
        console.log(
          `Mensualité ajoutée: Mois ${payment.month_number}, Montant: ${payment.payment_amount}`
        );
      }
      ...
      console.error(
        "Erreur lors de la création de la vente BNPL ______SERVICE_______ :",
        error
      );
  ...
  async advanceFundsToMerchant(sellerPubKey, amount) {
    try {
      ...
      console.log(
        `Transfert de ${advanceAmount} SOL au marchand avec signature : ${transactionSignature}`
      );
  ...
  async getUserBNPLSales(userPubKey) {
    try {
      ...
      throw new Error(
        "Impossible de récupérer les ventes BNPL de l'utilisateur"
      );
    }
  }
}
